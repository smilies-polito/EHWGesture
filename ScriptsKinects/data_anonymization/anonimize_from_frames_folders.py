...
def anonymize_with_sam(frame_files, face_boxes, hand_boxes, input_folder, video_writer, prompt_frames):
    state = predictor.init_state(input_folder)
    print("Using SAM to anonymize video...")
    
    for frame_idx, face_boxes_frame, hand_boxes_frame in zip(prompt_frames, face_boxes, hand_boxes):
        for id_b, box in enumerate(face_boxes_frame):
            print(f"Adding face box {box} to frame {frame_idx}")
            predictor.add_new_points_or_box(state, frame_idx=frame_idx, obj_id=id_b, box=box)
        for id_b, box in enumerate(hand_boxes_frame):
            print(f"Adding hand box {box} to frame {frame_idx}")
            predictor.add_new_points_or_box(state, frame_idx=frame_idx, obj_id=200 + id_b, box=box)
    ...
    for frame_idx, _, masks in predictor.propagate_in_video(state):
        ...
        face_masks = masks_np[0].clip(0, 1)
        hand_masks = masks_np[1].clip(0, 1)
...
def anonymize_with_fixed_bbox(frame_files, padded_face_boxes, video_writer):
    ...
    for frame_file in frame_files:
        ...
        for x1, y1, x2, y2 in padded_face_boxes[0]:
            blurred_region = cv2.medianBlur(base_frame[y1:y2, x1:x2], 31)
            base_frame[y1:y2, x1:x2] = blurred_region
        video_writer.write(base_frame)

def anonymize_frames(input_folder, output_folder, force_sam=False, iou_threshold=0.1, sam_prompts_every_n_frames=-1):
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        for idx, frame_file in enumerate(frame_files):
            if idx == 0 or (sam_prompts_every_n_frames > 0 and idx % sam_prompts_every_n_frames == 0):
                color_image = cv2.imread(frame_file)
                threshold = 0.5
                while threshold > 0:
                    results = model_face.predict(color_image[:, :, :3], conf=threshold, verbose=False)
                    new_face_boxes = [[int(x) for x in box.xyxy[0]] 
                        for result in results for box in result.boxes]
                    if new_face_boxes:
                        break
                    print(f"Warning: No face boxes detected at threshold {threshold}. Lowering threshold...")
                    threshold -= 0.1
                if not new_face_boxes:
                    print("Warning: No face boxes detected even at the lowest threshold.")

                hand_boxes.append(detect_hands(color_image, width, height))
                face_boxes.append(new_face_boxes)
                prompt_frames.append(idx)
                
                print(f"Detected {len(new_face_boxes)} faces and {len(hand_boxes[-1])} hands in frame {idx}")
        
        padded_face_boxes = []
        for face_boxes_frame in face_boxes:
            padded_face_boxes.append([[max(0, x1 - 40), max(0, y1 - 40), 
                                       min(width, x2 + 40), min(height, y2 + 40)]
                                      for x1, y1, x2, y2 in face_boxes_frame])

        use_sam = ('NOSE' in input_folder) or (force_sam)
        for frame_idx, (face_boxes_frame, hand_boxes_frame) in enumerate(zip(padded_face_boxes, hand_boxes)):
            if any(iou(hb, fb) > iou_threshold for hb in hand_boxes_frame for fb in face_boxes_frame):
                use_sam = True
                break
        
        if use_sam:
            anonymize_with_sam(frame_files, face_boxes, hand_boxes, input_folder, video_writer, prompt_frames)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_folder", type=str, default='example_dataset_frames\X05N_R\Prova_PSN1', help="Path to folder containing extracted frames.")
    parser.add_argument("--output_folder", type=str,default='out_anonimized', help="Path to output folder for anonymized videos.")
    parser.add_argument("--force_sam", action='store_true', help="Force using SAM for segmenting faces")
    parser.add_argument("--sam_prompts_every_n_frames", type=int, default=50, help="Run face/hand detection every n frames")
    ...
    for root, dirs, _ in os.walk(args.input_folder):
        for d in dirs:
            ...
            anonymize_frames(input_subfolder, output_subfolder, args.force_sam, sam_prompts_every_n_frames=args.sam_prompts_every_n_frames)
