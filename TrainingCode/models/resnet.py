...
class MultiInputResNet(nn.Module):
    def __init__(self, block, layers, sample_size, sample_duration, num_classes=400, input_heads=2):
        ...
        self.feature_extractors = nn.ModuleList([
            ResNet(block, layers, sample_size, sample_duration, num_classes=num_classes, include_top=False)
            for _ in range(input_heads)
        ])
        
        # breakpoint()
        # Adjust the classifier to handle concatenated features
        feature_size = 512 * block.expansion * input_heads
...
def generate_model(opt):
    ...
    if opt.width_mult != 1:
        print("Warning: width_mult != 1 not supported for ResNet, ignoring.")
